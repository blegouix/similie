...
A special class of tensors are the [antisymmetric tensors](https://en.wikipedia.org/wiki/Antisymmetric_tensor) which correspond to differential forms (ie. the [Faraday tensor](https://en.wikipedia.org/wiki/Electromagnetic_tensor) of electromagnetism), implemented in the \subpage exterior_module "Exterior module" of SimiLie. Even more specials are the [vectors](https://en.wikipedia.org/wiki/Vector_(mathematics_and_physics)) which correspond to 1-forms.
