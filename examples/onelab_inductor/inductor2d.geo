...
For j In {0:num_y - 1}
  For i In {0:num_x - 1}
    points[] += newp;
    Point(newp) = {x_coords[i], y_coords[j], 0, lc0};
  EndFor
EndFor
...
For j In {0:num_y - 2}
  For i In {0:num_x - 2}
    ...
    If(region_tag == ECORE)
      ecore_surfaces[] += surface_id;
    ElseIf(region_tag == ICORE)
      icore_surfaces[] += surface_id;
    ElseIf(region_tag == AIRGAP)
      airgap_surfaces[] += surface_id;
    ElseIf(region_tag == COIL)
      coil_left_surfaces[] += surface_id;
    ElseIf(region_tag == COIL + 1)
      coil_right_surfaces[] += surface_id;
    Else
      air_surfaces[] += surface_id;
    EndIf
  EndFor
EndFor

Physical Surface(ECORE) = {ecore_surfaces[]};
Physical Surface(ICORE) = {icore_surfaces[]};
Physical Surface(AIRGAP) = {airgap_surfaces[]};
Physical Surface(COIL) = {coil_left_surfaces[]};
Physical Surface(COIL + 1) = {coil_right_surfaces[]};
Physical Surface(AIR) = {air_surfaces[]};
