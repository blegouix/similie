...
template <class FillPosition>
ddc::DiscreteDomain<DDimX, DDimY> make_mesh_and_position(
        auto& position,
        FillPosition&& fill_position)
{
    ...
    ddc::host_for_each(mesh_xy, [&](ddc::DiscreteElement<DDimX, DDimY> elem) {
        ...
        fill_position(position, elem, xi, eta);
    });

    return mesh_xy;
}

template <class FormIndex, class FillPosition, class SetupForm, class CheckValue>
void run_reduction_test(
        FillPosition&& fill_position,
        SetupForm&& setup_form,
        CheckValue&& check_value)
{
    ...
    ddc::DiscreteDomain<DDimX, DDimY, PositionIndex>
            position_dom(ddc::DiscreteDomain<DDimX, DDimY>(), position_accessor.domain());
    ...
    position_dom
            = ddc::DiscreteDomain<DDimX, DDimY, PositionIndex>(mesh_xy, position_accessor.domain());
    ...
    ddc::Chunk reduced_alloc(form_dom, ddc::HostAllocator<double>());
...
void run_reconstruction_test(SetupForm&& setup_form, CheckValue&& check_value)
{
    ddc::Coordinate<X, Y> lower_bounds(0., 0.);
    ddc::Coordinate<X, Y> upper_bounds(2., 2.);
    ddc::DiscreteVector<DDimX, DDimY> nb_cells(2, 2);
    ...
    ddc::Chunk reduced_alloc(form_dom, ddc::HostAllocator<double>());
    sil::tensor::Tensor reduced(reduced_alloc);
    ddc::Chunk reconstructed_alloc(form_dom, ddc::HostAllocator<double>());
    sil::tensor::Tensor reconstructed(reconstructed_alloc);
    ...
    sil::exterior::fill_reconstruction_operator<
            IndexSeq>(Kokkos::DefaultHostExecutionSpace(), reconstruction_operator, position);
...
TEST(Reduction, Scalar)
{
    run_reduction_test<ScalarIndex>(
            [](auto position, auto elem, [[maybe_unused]] double xi, [[maybe_unused]] double eta) {
...
TEST(Reconstruction, Scalar)
{
    run_reconstruction_test<ScalarIndex>(
            [](auto form) { ddc::parallel_fill(form, 3.5); },
            [](auto reconstructed, auto elem) {
                EXPECT_DOUBLE_EQ(reconstructed(elem, ddc::DiscreteElement<ScalarIndex>(0)), 3.5);
            });
}

TEST(Reconstruction, OneForm)
{
    run_reconstruction_test<OneFormIndex>(
            [](auto form) {
                ddc::host_for_each(form.non_indices_domain(), [&](auto elem) {
                    form(elem, form.accessor().template access_element<X>()) = 3.;
                    form(elem, form.accessor().template access_element<Y>()) = -1.;
            ...
            [](auto reconstructed, auto elem) {
                EXPECT_DOUBLE_EQ(
                        reconstructed(elem, reconstructed.accessor().template access_element<X>()),
                        3.);
                EXPECT_DOUBLE_EQ(
                        reconstructed(elem, reconstructed.accessor().template access_element<Y>()),
                        -1.);
            });
}

TEST(Reconstruction, TwoForm)
{
    run_reconstruction_test<TwoFormIndex>(
            [](auto form) {
                ddc::host_for_each(form.non_indices_domain(), [&](auto elem) {
                    form(elem, form.accessor().template access_element<X, Y>()) = 4.;
                });
            ...
            [](auto reconstructed, auto elem) {
                EXPECT_DOUBLE_EQ(
                        reconstructed(
                                elem,
                                reconstructed.accessor().template access_element<X, Y>()),
                        4.);
            });
}
