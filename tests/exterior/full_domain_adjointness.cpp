...
TEST(AdjointCoboundary, FullDomainLaplacianIsWeightedSymmetric)
{
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            max_weighted_asymmetry
                    = std::max(max_weighted_asymmetry, std::abs(ma[i * n + j] - ma[j * n + i]));
        }
    }

    std::vector<double> sym_ma = ma;
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            double const avg = 0.5 * (ma[i * n + j] + ma[j * n + i]);
            sym_ma[i * n + j] = avg;
            sym_ma[j * n + i] = avg;
        }
    }
    std::vector<double> eigs = jacobi_eigenvalues(sym_ma, n);

    EXPECT_LT(max_constant_residual, 1e-12);
    EXPECT_LT(max_weighted_asymmetry, 1e-12);
    EXPECT_GT(eigs.front(), -1e-10);
