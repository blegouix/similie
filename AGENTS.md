...
- **Default build** By default (no particular contradictory instructions), you will check if a GPU is available with `lshw` from the docker image. If so, you will try to build Docker image and compile for it; otherwise, build Docker image and compile for CPU. The CPU backend must be selected explicitly with `--build-arg BACKEND=cpu`.
