...
- **Default build** By default (no particular contradictory instructions), you will check if a GPU is available with `lshw` from the docker image. If so, you will try to build Docker image and compile for it; otherwise, build Docker image and compile for CPU.
- **Adapt Docker base image** By default (no particular contradictory instructions), when building the CPU image, pass `--build-arg BASE_IMAGE=ubuntu:24.04` to `docker build` instead of using the NVIDIA CUDA image.
