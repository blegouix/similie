...
- **Adapt Docker base image** By default (no particular contradictory instructions), the CPU backend always uses the official Ubuntu 26 base image. In practice, when building the CPU image, always pass both `--build-arg BACKEND=cpu` and `--build-arg BASE_IMAGE=ubuntu:26.04` to `docker build` instead of using the NVIDIA CUDA image. You will call the CPU image `similie_env_cpu`.
